  ...
  return (
    <div style={{ background: 'var(--color-canvas)', minHeight: '100vh', fontFamily: "'Hanken Grotesk'" }}>
      ...
      <style>{`
        ...
        .sk-opt-btn:not(.active):hover { border-color: var(--color-brand) !important; background: #F8FAF0 !important; transform: translateY(-1px); box-shadow: 0 4px 12px rgba(94,106,42,.12); }
      ...
      <div className="sk-q-layout" style={{ display: 'grid', gridTemplateColumns: '1fr 300px', minHeight: 'calc(100vh - 80px)', alignItems: 'stretch' }}>
        ...
        {/* ── Left: questions ── */}
        <div className="sk-q-main" style={{ padding: '38px 52px 60px', borderRight: '1px solid var(--color-field-border)' }}>

          {/* Step badge */}
          <div style={{ display: 'inline-flex', alignItems: 'center', gap: 8, background: 'var(--color-ink)', borderRadius: '999px', padding: '4px 13px', marginBottom: 18 }}>
          ...
          <h2 style={{ fontFamily: "'Newsreader',serif", fontWeight: 400, fontSize: 36, letterSpacing: '-.02em', margin: '0 0 8px', color: 'var(--color-ink)', lineHeight: 1.15 }}>
            {isMedium ? 'Help us validate your result.' : 'A few details to tailor your remedies.'}
          </h2>
          <p style={{ fontSize: 14.5, color: 'var(--color-body)', margin: '0 0 24px', maxWidth: 500, lineHeight: 1.65 }}>
            {isMedium
          ...
          {isMedium && (
            <div style={{ background: 'var(--color-warn-bg)', border: '1.5px solid var(--color-warn-border)', borderLeft: '4px solid #F0C040', borderRadius: 13, padding: '14px 18px', marginBottom: 24, display: 'flex', gap: 14, alignItems: 'flex-start' }}>
              <div style={{ width: 32, height: 32, borderRadius: 9, background: '#F5E58A', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="var(--color-warn-strong)" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><path d="M10.29 3.86L1.82 18a2 2 0 0 0 1.71 3h16.94a2 2 0 0 0 1.71-3L13.71 3.86a2 2 0 0 0-3.42 0z"/><line x1="12" y1="9" x2="12" y2="13"/><line x1="12" y1="17" x2="12.01" y2="17"/></svg>
              </div>
              <div>
                <div style={{ fontWeight: 700, fontSize: 13.5, color: 'var(--color-warn-strong)', marginBottom: 3 }}>Moderate AI Confidence — Validation Required</div>
          ...
          <div style={{ marginBottom: 32 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
              <span style={{ fontFamily: "'Spline Sans Mono',monospace", fontSize: 10, letterSpacing: '.12em', textTransform: 'uppercase', color: 'var(--color-muted)' }}>Progress</span>
              <span style={{ fontFamily: "'Spline Sans Mono',monospace", fontSize: 11, color: progress === 100 ? 'var(--color-brand-text)' : 'var(--color-muted)', fontWeight: 500 }}>
            ...
            <div style={{ height: 8, background: '#E4E1D4', borderRadius: 999, overflow: 'hidden', border: '1px solid #D8D4C4' }}>
              <div style={{ height: '100%', width: `${progress}%`, background: progress === 100 ? 'var(--color-success)' : 'var(--color-brand)', borderRadius: 999, transition: 'width .35s ease, background .3s' }} />
            </div>
          </div>

          {/* ── Section 1: Lifestyle ── */}
          <SectionBanner number="1" label="Lifestyle" color="var(--color-brand-text)" bg="var(--color-surface-tint)" border="var(--color-header-line)"
          ...
          <SectionBanner number="2" label="AI Prediction Validation" color="var(--color-warn-strong)" bg="var(--color-warn-bg)" border="var(--color-warn-border)"
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 16, marginTop: 36 }}>
            <button
              ...
              style={{
                background: answered >= totalQ ? 'var(--color-brand)' : 'var(--color-tint-neutral)',
                color:      answered >= totalQ ? 'var(--color-brand-ink)' : 'var(--color-muted)',
            ...
            <span style={{ fontSize: 13, color: answered >= totalQ ? 'var(--color-brand-text)' : 'var(--color-muted)', fontWeight: answered >= totalQ ? 600 : 400 }}>
              {answered < totalQ
          ...
          <div style={{ position: 'sticky', top: 80, height: 'calc(100vh - 80px)', overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
            ...
            <div style={{ position: 'relative', zIndex: 1, display: 'flex', flexDirection: 'column', height: '100%', padding: '32px 24px' }}>
              ...
              <div style={{ background: 'rgba(255,255,255,.72)', border: '1.5px solid #D4DEB8', borderRadius: 14, padding: '16px 18px', marginBottom: 'auto', backdropFilter: 'blur(8px)' }}>
                <div style={{ fontFamily: "'Spline Sans Mono',monospace", fontSize: 9.5, letterSpacing: '.13em', textTransform: 'uppercase', color: 'var(--color-success)', marginBottom: 10 }}>AI Detection</div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 10, padding: '24px 0' }}>
                ...
                <div style={{ fontFamily: "'Newsreader',serif", fontSize: 17, color: 'var(--color-brand-text)', textAlign: 'center', lineHeight: 1.3 }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                {[
                  ...
                  <div key={num} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 12px', background: 'rgba(255,255,255,.65)', border: '1px solid #D4DEB8', borderRadius: 10, backdropFilter: 'blur(6px)' }}>
                    <div style={{ width: 26, height: 26, borderRadius: '50%', background: 'var(--color-surface-tint)', border: '1.5px solid var(--color-brand)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                      <span style={{ fontFamily: "'Spline Sans Mono',monospace", fontSize: 10, color: 'var(--color-brand-text)', fontWeight: 700 }}>{num}</span>
                    </div>
                    <span style={{ fontSize: 12.5, color: 'var(--color-text2)', flex: 1 }}>Section {num} · {label}</span>
                    <span style={{ fontFamily: "'Spline Sans Mono',monospace", fontSize: 10, color: 'var(--color-success)' }}>{count}Q</span>
                  </div>
                ))}
              </div>
            </div>
    ...
    <div style={{ margin: '32px 0 16px' }}>
      <div style={{ display: 'inline-flex', alignItems: 'center', gap: 10, background: bg, border: `1.5px solid ${border}`, borderRadius: '999px', padding: '7px 16px 7px 8px', marginBottom: desc ? 8 : 0 }}>
        <div style={{ width: 26, height: 26, borderRadius: '50%', background: color, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
          <span style={{ fontFamily: "'Spline Sans Mono',monospace", fontSize: 11, color: 'var(--color-surface)', fontWeight: 700 }}>{number}</span>
        </div>
  ...
  return (
    <div className="sk-q-card"
      style={{
        ...
        border: `1.5px solid ${active ? 'var(--color-header-line)' : 'var(--color-field-border)'}`,
        borderLeft: `4px solid ${active ? 'var(--color-brand)' : '#D4D0C4'}`,
        borderRadius: 16,
        padding: '18px 20px',
    ...
    >
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', gap: 14, marginBottom: 14 }}>
        {/* Number badge */}
        <div style={{ width: 34, height: 34, borderRadius: '50%', background: active ? 'var(--color-brand)' : 'var(--color-tint-neutral)', border: `2px solid ${active ? '#8A9A40' : '#D4D0C4'}`, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, transition: 'all .2s' }}>
          {active
            ? <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="var(--color-brand-ink)" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round"><polyline points="20 6 9 17 4 12"/></svg>
            : <span style={{ fontFamily: "'Spline Sans Mono',monospace", fontSize: 11, color: 'var(--color-muted)', fontWeight: 700, lineHeight: 1 }}>{String(qi + 1).padStart(2, '0')}</span>
          }
        </div>

        {/* Question text + category */}
        <div style={{ flex: 1 }}>
          <span style={{ fontSize: 14.5, fontWeight: 600, color: 'var(--color-ink)', lineHeight: 1.45 }}>{q.question}</span>
        </div>

        <span style={{ fontFamily: "'Spline Sans Mono'", fontSize: 9, letterSpacing: '.08em', textTransform: 'uppercase', color: '#9AA646', background: 'var(--color-surface-tint)', padding: '4px 9px', borderRadius: '999px', whiteSpace: 'nowrap', flexShrink: 0, border: '1px solid #D8DFA8' }}>
          {q.category}
        </span>
      </div>
          ...
          return (
            <button
              ...
              style={{
                ...
                border: isActive ? '2px solid var(--color-brand)' : '1.5px solid var(--color-field-border)',
                background: isActive ? '#F4F6E8' : 'var(--color-surface)',
                color: isActive ? '#4A5820' : 'var(--color-text2)',
                boxShadow: isActive ? '0 2px 10px rgba(190,202,92,.25)' : 'none',
              }}
            >
              {isActive && (
                <span style={{ display: 'block', fontSize: 9, color: 'var(--color-success)', marginBottom: 3, fontFamily: "'Spline Sans Mono',monospace", letterSpacing: '.08em' }}>SELECTED</span>
              )}
              {opt}
            </button>
          );
  ...
  return (
    <div style={{ position: 'relative', width: 110, height: 110 }}>
      <svg width="110" height="110" style={{ transform: 'rotate(-90deg)' }}>
        ...
        <circle cx="55" cy="55" r={r} fill="none" stroke="var(--color-brand)" strokeWidth="8"
          strokeDasharray={c} strokeDashoffset={offset}
          strokeLinecap="round" style={{ transition: 'stroke-dashoffset .4s ease' }} />
      </svg>
      <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}>
        <span style={{ fontFamily: "'Newsreader',serif", fontSize: 26, color: 'var(--color-brand-text)', lineHeight: 1 }}>{progress}%</span>
        <span style={{ fontFamily: "'Spline Sans Mono',monospace", fontSize: 9, color: 'var(--color-muted)', letterSpacing: '.08em', marginTop: 2 }}>{answered}/{totalQ}</span>
      </div>
    </div>
  );
}
