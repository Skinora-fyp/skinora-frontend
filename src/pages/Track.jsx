...
const ADVICE_PALETTE = [
  { bg: 'linear-gradient(135deg,#EEF4DC,#E4ECC8)', border: '#B8CC70', icon: 'var(--color-brand-text)', text: '#3A4818' },
...
const FEATURES = [
  { Icon: CameraIcon, title: 'Photo comparison', desc: 'Before & after your remedy', grad: 'var(--color-surface-tint)' },
    ...
    return (
      <div style={{ background: 'var(--color-canvas)', minHeight: '100vh', fontFamily: "'Hanken Grotesk'" }}>
        ...
        <main className="sk-in" style={{ maxWidth: 460, margin: '0 auto', padding: '56px 24px 56px', textAlign: 'center' }}>
          <div style={{ width: 64, height: 64, borderRadius: '50%', background: 'var(--color-surface-tint)', border: '2px solid var(--color-header-line)', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 20px', color: 'var(--color-brand-text)', boxShadow: '0 6px 20px rgba(190,202,92,.22)' }}>
            <LeafIcon />
          </div>
          <div style={{ fontFamily: "'Spline Sans Mono',monospace", fontSize: 10, letterSpacing: '.14em', textTransform: 'uppercase', color: 'var(--color-success)', marginBottom: 10 }}>You're all set</div>
          <h2 style={{ fontFamily: "'Newsreader',serif", fontWeight: 400, fontSize: 30, letterSpacing: '-.02em', margin: '0 0 12px', color: 'var(--color-ink)' }}>Your remedy plan is ready.</h2>
          <p style={{ fontSize: 13.5, color: 'var(--color-body)', lineHeight: 1.65, margin: '0 auto 22px', maxWidth: 340 }}>
          ...
          <div style={{ background: 'linear-gradient(135deg,#EEF4DC,#E4ECC8)', border: '1.5px solid var(--color-header-line)', borderRadius: 14, padding: '14px 18px', marginBottom: 24, display: 'flex', alignItems: 'center', gap: 12, textAlign: 'left' }}>
            <div style={{ width: 30, height: 30, borderRadius: '50%', background: 'var(--color-surface)', border: '1.5px solid var(--color-header-line)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, color: 'var(--color-brand-text)' }}><CheckCircle /></div>
            <div>
              <div style={{ fontSize: 13.5, fontWeight: 700, color: '#3A4818' }}>Remedy saved to your profile</div>
              <div style={{ fontSize: 12, color: 'var(--color-brand-text)', marginTop: 2 }}>{remedy?.name ?? 'Selected'}{condition ? ` · ${condition.replace('_',' ')}` : ''}</div>
            </div>
          </div>
          <div style={{ display: 'flex', gap: 10, justifyContent: 'center', flexWrap: 'wrap' }}>
            <button onClick={() => navigate('/upload')} className="sk-lime-btn" style={{ background: 'var(--color-brand)', color: 'var(--color-brand-ink)', border: '1.5px solid #8A9A40', borderRadius: '999px', padding: '12px 26px', fontFamily: "'Hanken Grotesk'", fontWeight: 700, fontSize: 14, cursor: 'pointer', boxShadow: '0 4px 16px rgba(190,202,92,.32)' }}>Go to dashboard →</button>
            <button onClick={() => navigate('/history')} className="sk-out-btn" style={{ background: 'transparent', border: '1.5px solid var(--color-field-border)', color: 'var(--color-text2)', borderRadius: '999px', padding: '12px 20px', fontFamily: "'Hanken Grotesk'", fontWeight: 600, fontSize: 13.5, cursor: 'pointer' }}>View history</button>
          </div>
        </main>
      </div>
    );
  ...
  return (
    <div style={{ background: 'var(--color-canvas)', minHeight: '100vh', fontFamily: "'Hanken Grotesk'" }}>
      ...
      <main style={{ maxWidth: 720, margin: '0 auto', padding: '24px 28px 52px' }}>
        ...
        {/* Remedy banner */}
        <div className="sk-in" style={{ background: 'linear-gradient(135deg,#EEF4DC,#E4ECC8)', border: '1.5px solid var(--color-header-line)', borderRadius: 14, padding: '13px 18px', display: 'flex', alignItems: 'center', gap: 12, marginBottom: 18, boxShadow: '0 3px 12px rgba(190,202,92,.15)' }}>
          <div style={{ width: 30, height: 30, borderRadius: '50%', background: 'var(--color-surface)', border: '1.5px solid var(--color-header-line)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, color: 'var(--color-brand-text)' }}><CheckCircle /></div>
          <div>
            <div style={{ fontSize: 11, fontFamily: "'Spline Sans Mono',monospace", letterSpacing: '.09em', textTransform: 'uppercase', color: 'var(--color-success)', marginBottom: 2 }}>Remedy selected</div>
        ...
        {advices.length > 0 && (
          <div className="sk-in" style={{ marginBottom: 22, animationDelay: '60ms' }}>
            {/* Section header */}
            <div style={{ background: 'linear-gradient(135deg,#23241C,#5E6A2A)', borderRadius: '14px 14px 0 0', padding: '14px 20px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 9 }}>
                <div style={{ width: 28, height: 28, borderRadius: '50%', background: 'rgba(190,202,92,.22)', border: '1px solid rgba(190,202,92,.45)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--color-brand)' }}><SparkIcon /></div>
                <div>
              ...
              <span style={{ background: 'rgba(190,202,92,.18)', border: '1px solid rgba(190,202,92,.35)', color: 'var(--color-brand)', fontFamily: "'Spline Sans Mono',monospace", fontSize: 9, letterSpacing: '.08em', textTransform: 'uppercase', padding: '4px 10px', borderRadius: '999px' }}>
                {advices.length} tip{advices.length > 1 ? 's' : ''}
              </span>
            </div>
            {/* Advice cards */}
            <div style={{ border: '1.5px solid var(--color-hairline)', borderTop: 'none', borderRadius: '0 0 14px 14px', overflow: 'hidden' }}>
              {advices.map((advice, i) => {
                const pal = ADVICE_PALETTE[i % ADVICE_PALETTE.length];
                return (
                  <div key={advice.tag} className="sk-advice-row"
                    style={{ display: 'flex', gap: 14, alignItems: 'flex-start', padding: '14px 18px', background: 'var(--color-surface)', borderBottom: i < advices.length - 1 ? '1px solid var(--color-hairline)' : 'none' }}>
                    ...
                    <div style={{ flex: 1 }}>
                      ...
                      <p style={{ fontSize: 13.5, color: 'var(--color-ink)', lineHeight: 1.58, margin: 0 }}>{advice.text}</p>
                    </div>
                  </div>
                );
              })}
        ...
        {phase === 'ask' && (
          <div className="sk-in" style={{ animationDelay: '110ms' }}>
            <div style={{ display: 'inline-flex', alignItems: 'center', gap: 7, background: 'var(--color-ink)', borderRadius: '999px', padding: '3px 12px', marginBottom: 11 }}>
              <span style={{ width: 5, height: 5, borderRadius: '50%', background: 'var(--color-brand)', display: 'inline-block' }} />
              <span style={{ fontFamily: "'Spline Sans Mono',monospace", fontSize: 9.5, letterSpacing: '.12em', textTransform: 'uppercase', color: 'var(--color-brand)' }}>Step 6 · Progress Tracking</span>
            </div>
            <h2 style={{ fontFamily: "'Newsreader',serif", fontWeight: 400, fontSize: 32, letterSpacing: '-.02em', margin: '0 0 8px', color: 'var(--color-ink)', lineHeight: 1.15 }}>
              Want to track your progress?
            </h2>
            <p style={{ fontSize: 14, color: 'var(--color-body)', lineHeight: 1.65, maxWidth: 460, margin: '0 0 20px' }}>
            ...
            <div className="sk-features" style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 12, marginBottom: 22 }}>
              {FEATURES.map(({ Icon, title, desc, grad }, i) => (
                <div key={title} className="sk-feature" style={{ background: grad, border: '1.5px solid var(--color-field-border)', borderRadius: 13, padding: '16px 14px', textAlign: 'center', boxShadow: '0 2px 10px rgba(35,36,28,.07)', animationDelay: `${130 + i * 55}ms` }}>
                  ...
                  <div style={{ fontWeight: 700, fontSize: 13, color: 'var(--color-ink)', marginBottom: 4 }}>{title}</div>
            ...
            <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
              <button onClick={() => setShowModal(true)} className="sk-lime-btn" style={{ background: 'var(--color-brand)', color: 'var(--color-brand-ink)', border: '1.5px solid #8A9A40', borderRadius: '999px', padding: '13px 28px', fontFamily: "'Hanken Grotesk'", fontWeight: 700, fontSize: 14.5, cursor: 'pointer', boxShadow: '0 4px 16px rgba(190,202,92,.32)' }}>
                Yes, track my progress →
              </button>
              <button onClick={declineTracking} className="sk-out-btn" style={{ background: 'transparent', border: '1.5px solid var(--color-field-border)', color: 'var(--color-text2)', borderRadius: '999px', padding: '13px 20px', fontFamily: "'Hanken Grotesk'", fontWeight: 600, fontSize: 13.5, cursor: 'pointer' }}>
                No thanks, I'm done
              </button>
            </div>
          </div>
      ...
      {showModal && (
        <>
          ...
          {/* Modal card */}
          <div className="sk-modal" style={{ position: 'fixed', top: '50%', left: '50%', transform: 'translate(-50%,-50%)', zIndex: 201, width: 'min(480px, calc(100vw - 40px))', background: 'var(--color-surface)', borderRadius: 20, boxShadow: '0 24px 64px rgba(26,30,10,.28), 0 4px 16px rgba(26,30,10,.12)', overflow: 'hidden' }}>

            {/* Modal header */}
            <div style={{ background: 'linear-gradient(135deg,#23241C,#5E6A2A)', padding: '20px 24px 18px' }}>
            ...
            <div style={{ padding: '20px 24px 24px' }}>
                ...
                {[
                  ...
                  return (
                    <button key={value} onClick={() => setFreq(value)} className="sk-freq-btn"
                      style={{ flex: 1, borderRadius: 13, border: `2px solid ${active ? 'var(--color-brand)' : 'var(--color-field-border)'}`, background: active ? grad : 'var(--color-surface)', padding: '14px 16px', textAlign: 'left', boxShadow: active ? '0 4px 16px rgba(190,202,92,.22)' : 'none', cursor: 'pointer' }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 5 }}>
                        <span style={{ fontFamily: "'Newsreader',serif", fontSize: 18, color: 'var(--color-ink)' }}>{label}</span>
                        <span style={{ width: 18, height: 18, borderRadius: '50%', border: `2px solid ${active ? 'var(--color-brand)' : '#CFCBBC'}`, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, background: active ? 'var(--color-brand)' : 'transparent', transition: 'all .15s' }}>
                          {active && <svg width="8" height="8" viewBox="0 0 24 24" fill="none" stroke="var(--color-brand-ink)" strokeWidth="3.5" strokeLinecap="round" strokeLinejoin="round"><polyline points="20 6 9 17 4 12"/></svg>}
                        </span>
                      </div>
                      ...
                      <p style={{ fontSize: 12, color: 'var(--color-body)', lineHeight: 1.45, margin: 0 }}>{desc}</p>
                    </button>
                  );
                })}
              </div>
              ...
              {/* Email summary */}
              <div style={{ background: 'linear-gradient(135deg,#EEF4DC,#E4ECC8)', border: '1.5px solid var(--color-header-line)', borderRadius: 11, padding: '11px 14px', display: 'flex', alignItems: 'center', gap: 11, marginBottom: 20 }}>
                <div style={{ width: 32, height: 32, borderRadius: 9, background: 'var(--color-surface)', border: '1.5px solid var(--color-header-line)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, color: 'var(--color-brand-text)' }}><MailIcon /></div>
                <div>
              ...
              <div style={{ display: 'flex', gap: 10 }}>
                <button onClick={confirmTracking} disabled={loading} className="sk-lime-btn"
                  style={{ flex: 1, background: loading ? '#D8DC9A' : 'var(--color-brand)', color: 'var(--color-brand-ink)', border: '1.5px solid #8A9A40', borderRadius: '999px', padding: '13px 20px', fontFamily: "'Hanken Grotesk'", fontWeight: 700, fontSize: 14, cursor: loading ? 'default' : 'pointer', boxShadow: '0 4px 16px rgba(190,202,92,.3)', opacity: loading ? 0.8 : 1 }}>
                  {loading ? 'Setting up…' : `Confirm — ${freq === 'weekly' ? 'every week' : 'every month'} →`}
                </button>
                <button onClick={() => setShowModal(false)} className="sk-out-btn"
                  style={{ background: 'transparent', border: '1.5px solid var(--color-field-border)', color: 'var(--color-text2)', borderRadius: '999px', padding: '13px 18px', fontFamily: "'Hanken Grotesk'", fontWeight: 600, fontSize: 13, cursor: 'pointer', whiteSpace: 'nowrap' }}>
                  Cancel
                </button>
              </div>
            </div>
...
.sk-freq-btn:hover { border-color: var(--color-brand) !important; }
